...
#include "lib/nec_protocol.h"

static uint8_t packet_reading_state = PACKET_STATE_NO_PACKET;
static uint8_t packet_writing_state = PACKET_STATE_NO_PACKET;
...
static uint8_t write_bit_counter = 0;
...
void reset_packet()
{
	...
	packet_writing_state = PACKET_STATE_NO_PACKET;
}
...
void start_IR_timer(uint8_t time_ms)
{
	...
	OCR1A = ((uint32_t) time_ms * 1000)/2;  // it makes delay = 16 ms when Fcpu = 16 Mhz, it makes the delay enough for reading each bit according to NEC	
